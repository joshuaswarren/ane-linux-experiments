...
Three phases:
  ...
  C. scale: N = 2,4,8 identical TDs, wall time per tile, compared with the
     one-ioctl-per-tile cost measured in m1-test-host-ane-tiled.py (1.93 ms).
...
TD layout facts used here (from gemm.py and concat.py):
  ...
  usage: python3 m1-test-host-ane-batch.py
"""
import os
import struct
import time
...
GEMM = "$HOME/src/apple-ane/examples/gemm.py"
src = open(GEMM).read()
...
def main():
    ...
    print("baseline per_tile from m1-test-host-ane-tiled.py: 1.93 ms")
    for n in (2, 4, 8):
        tsk_size = (n - 1) * TD_SPACING + TD_SIZE
        cmd = make_cmd(tsk_size, [np.float16(0.5)])
        ret, wall, out = run(n, [base_td] * n, cmd, OUT_BYTES)
