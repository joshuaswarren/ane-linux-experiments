...
tiles / 6832128 bytes and returned 0 wrong lanes on t8103-host. ``--islands`` selects
...
class _IslandFuture:
    """One pending async island submit; resolves to the packed output array."""
    ...
    def resolve(self, value: "mx.array") -> None:
        self._value = value
        self._event.set()
...
def parse_kwargs(text: str) -> dict[str, str]:
    ...
    for item in split_top(text):
        ...
        value = value.strip()
        # The protobuf re-export writes attr strings as string("x");
        # the original adapter wrote bare "x". Normalize both.
...
class Blobs:
    ...
    def payload_span(self, path: str, offset: int, want: int) -> int:
        """Resolve a blob header; returns the payload byte offset."""
        raw = self._map(path)
        magic, _storage, length, payload = struct.unpack_from(
            "<IIQQ", raw[offset : offset + 24].tobytes(), 0
        )
    ...
    def dtype_view(self, path: str, np_dtype):
        """Whole blob file as one device array + host mirror, per dtype.
        Consts slice it instead of each paying an array creation + upload
        (~0.1 ms x ~2000 consts per pass on t8103-host = ~0.7 s)."""
        key = (path, np.dtype(np_dtype).str)
        if key not in self._views:
            host = np.frombuffer(self._map(path), dtype=np_dtype)
            self._views[key] = (mx.array(host), host)
        return self._views[key]


_stmt_trace = os.environ.get("MLX_OMARCHY_STMT_TRACE", "0") == "1"
...
class AneIsland:
    ...
    launch -- one process per submit: the worker CLI takes a
    ...
    inprocess -- the worker's own submission code (libane device, bundle
    parse, manifest validation, tile layout) linked into this process via
    the ane_inproc shim (ANE_INPROC_SHIM), addressed through ctypes: no
    worker child, no pipe IPC. Same safety contract as resident-batch,
    preserved by other means: submits run on the shim's persistent thread
    bounded by the batch deadline, the host quarantine file is checked
    before every submit, and a timeout or failure quarantines the session
    and refuses every later submit, never retried. ANE_ISLAND_MODE=resident
    -batch restores the worker path as fallback.
    """
    ...
    def __init__(self, worker: Path, libane: Path, bundles: Path, scratch: Path,
        ...
        self.log: list[dict] = []
        # Default transport is the serve worker (spawn paid once): t6001-host
        ...
        if self._mode not in ("launch", "resident-batch", "inprocess"):
            raise EncoderRunError(
                f"ANE_ISLAND_MODE {self._mode!r} is not launch, "
                "resident-batch, or inprocess"
            )
        self._batch_deadline_ms = int(
            os.environ.get("ANE_ISLAND_BATCH_DEADLINE_MS", "120000")
        )
        # Fused A->B package (add + select composed, EHC out_ab bundle).
        self.fused_ab = os.environ.get("MLX_OMARCHY_FUSED_AB", "0") == "1"
        self._session = None
        self._inproc = None
        self._inproc_open_ns = 0
        self.feeder = None
        # Cross-layer pipelining (ANE_ASYNC_ISLANDS=1, inprocess mode only):
        # island marshal+submit runs on one FIFO worker thread, so layer
        # N's ANE execution overlaps the next island's marshal wait (GPU
        # feeder drain) instead of stalling the main issue thread. ANE
        # submits stay strictly ordered; the quarantine/deadline contract
        # is the shim's, unchanged.
        self.pipeline = (
            os.environ.get("ANE_ASYNC_ISLANDS", "0") == "1"
            and self._mode == "inprocess"
        )
        self._pipe_q: queue.SimpleQueue | None = None
        self._pipe_thread: threading.Thread | None = None

    def session_bundles(self) -> list:
        """Named bundles one session loads, including the fused A->B
        package (add + select) when MLX_OMARCHY_FUSED_AB is set."""
        names = list(RESIDENT_BUNDLES)
        if self.fused_ab:
            names.append("out_ab")
        return names
    ...
    def _ensure_session(self):
        ...
        session = ResidentAneWorker(
            ...
            bundles={
                ...
                for name in self.session_bundles()
            },
    ...
    def _pipeline_worker(self) -> None:
        # Device phase only: no MLX graph work here (streams are
        # thread-local, so mx.eval of main-built graphs is illegal on this
        # thread). ctypes shim submit releases the GIL; mx.array from a
        # host buffer needs no stream.
        while True:
            job = self._pipe_q.get()
            if job is None:
                return
            bundle, tag, payload, arrays, outputs, in_bytes, drains, marshal_ns, feeder, futs = job
            try:
                inproc = self._ensure_inprocess()
                started = time.monotonic_ns()
                inproc.submit(bundle, payload, arrays)
                elapsed = time.monotonic_ns() - started
                self.rounds += 1
                self.exec_ns += elapsed
                self.input_bytes += in_bytes
                record = {
                    ...
                    "marshal_drains_ns": drains,
                    "dev_send_ns": inproc.last_send_ns,
                    "dev_exec_ns": inproc.last_exec_ns,
                    "dev_read_ns": inproc.last_read_ns,
                }
                if feeder is not None:
                    record["feeder_issue_ns"] = feeder["issue_ns"]
                    record["feeder_ops"] = feeder["ops"]
                out_bytes = 0
                packed = {}
                for name, (shape, dtype_name) in outputs.items():
                    host = arrays[name]
                    ...
                    for dim in shape:
                        count *= dim
                    expect = count * np.dtype(NP_DTYPES[dtype_name]).itemsize
                    if host.nbytes != expect:
                        raise EncoderRunError(
                            f"ANE output {name} for {tag} is {host.nbytes} bytes, want {expect}"
                        )
                    out_bytes += expect
                    packed[name] = mx.array(host).reshape(shape)
                record["output_bytes"] = out_bytes
                ...
                for name, future in futs.items():
                    future.resolve(packed[name])
            except BaseException as error:  # rethrown at future.wait()
                for future in futs.values():
                    future.fail(error)

    def submit(self, bundle: str, tag: str, inputs: dict, outputs: dict) -> dict:
        """inputs: name -> mx array. outputs: name -> (shape, dtype name).

        With pipelining on (inprocess only), the marshal (mx.eval of the
        input graph) stays on the calling thread -- MLX forbids eval of
        main-built graphs elsewhere -- and only the device submit runs on
        the FIFO pipeline thread. Returns one _IslandFuture per output;
        the caller stores the futures in the value table and reads
        resolve them.
        """
        if self.pipeline:
            if self._pipe_thread is None:
                ...
                self._pipe_thread = threading.Thread(
                    target=self._pipeline_worker, name="ane-pipeline", daemon=True
                )
                self._pipe_thread.start()
            payload, in_bytes, drains, marshal_ns, feeder = self._marshal_inputs(inputs)
            arrays = {}
            for name, (shape, dtype_name) in outputs.items():
                count = 1
                for dim in shape:
                    count *= dim
                arrays[name] = np.empty(count, dtype=NP_DTYPES[dtype_name])
            futs = {name: _IslandFuture() for name in outputs}
            self._pipe_q.put(
                (bundle, tag, payload, arrays, outputs, in_bytes, drains,
                 marshal_ns, feeder, futs)
            )
            return futs
        if self._mode == "inprocess":
            return self._submit_inprocess(bundle, tag, inputs, outputs)
    ...
    def _ensure_inprocess(self):
        if self._inproc is not None:
            return self._inproc
        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from ane_inproc import InProcessAne, quarantine_set
        if quarantine_set():
            raise EncoderRunError(
                "ANE runtime is quarantined for this boot; reboot required"
            )
        shim = Path(os.environ.get(
            "ANE_INPROC_SHIM",
            Path(__file__).resolve().parent / "libane_inproc.so",
        ))
        started = time.monotonic_ns()
        inproc = InProcessAne(
            shim=shim,
        ...
        inproc.begin_batch(self._batch_deadline_ms)
        self._inproc_open_ns = time.monotonic_ns() - started
        self.worker_starts += 1
        self.submissions += 1
        self._inproc = inproc
        return inproc

    def _marshal_inputs(self, inputs: dict):
        """Marshal island inputs on the CALLING thread: mx.eval may only run
        on the thread that built the graph (streams are thread-local in
        MLX), so pipelining never moves this phase off the main thread."""
        payload = {}
        in_bytes = 0
        marshal0 = time.monotonic_ns()
        drains: dict[str, int] = {}
        for name, value in inputs.items():
            _m0 = time.monotonic_ns()
            mx.eval(value)
            drains[name] = time.monotonic_ns() - _m0
            raw = np.ascontiguousarray(np.asarray(value)).tobytes()
            payload[name] = raw
            in_bytes += len(raw)
        marshal_ns = time.monotonic_ns() - marshal0
        feeder, self.feeder = self.feeder, None
        return payload, in_bytes, drains, marshal_ns, feeder

    def _submit_inprocess(self, bundle: str, tag: str, inputs: dict, outputs: dict) -> dict:
        """One bounded submit into the in-process shim: no pipe, no child."""
        payload, in_bytes, drains, marshal_ns, feeder = self._marshal_inputs(inputs)
        inproc = self._ensure_inprocess()
        started = time.monotonic_ns()
        arrays = {}
        try:
            ...
            raise EncoderRunError(
                f"ANE in-process submit {tag} ({bundle}) failed: {error}"
            ) from error
        elapsed = time.monotonic_ns() - started
        self.rounds += 1
        self.exec_ns += elapsed
...
class EncoderRunner:
    def __init__(self, mil_path: Path, model_root: Path, island: AneIsland | None,
        ...
        # Default on (measured t6001-host: AC launch -1185ms, resident -813ms,
        ...
        # Coarser issue cadence beats per-statement (t6001-host sweep: conv-only
        # 6561/5867 vs all-ops 7655/7179 AC launch/resident) - async_eval at
        # conv statements only. Host-dependent: with the in-process submit
        # the t6001-host GPU wants all-ops issue (encoder_ane 1228.7/1260.1 ms vs
        # 1717-1868 conv-only, 6 meas all pins green) but the t8103-host e167
        # asahi driver serializes pathologically under all-ops issue
        # (encoder_ane 32755 ms vs 8556 conv-only, same battery). Default
        # stays conv-only everywhere; MLX_OMARCHY_PIPE_OPS= opts a host in.
        ...
        # Feeder instrumentation (question 1): per-op CPU issue wall and
        # the ops accumulated between island submits. Reset per island.
        self.seg_issue_ns = 0
        self.seg_ops: dict[str, list] = {}
        self.seg_eval_ns = 0
        self.island_feeder = None
        ...
        self._index_fusions()
        self._last_use()
        # Fused A->B (MLX_OMARCHY_FUSED_AB=1): the add producing the
        # select's b operand stops running on the GPU; the fused out_ab
        # bundle consumes its operands directly and emits the masked
        # attention scores.
        self.fused_ab = island is not None and island.fused_ab
        self.fused_ab_adds: dict[int, bool] = {}
        if self.fused_ab:
            # j16 lineage: the select's b comes straight from an add, and the
            # bundle consumes the add's operands. t8103-host lineage ("bdscale"):
            # b = mul(relpos_slice, scalar); the scalar-mul output crosses
            # unscaled-by-the-bundle as s2 and the consumer add folds it into
            # the scores. In bdscale mode the bundle's z replaces the CONSUMER
            # add's output and both the select and the add are skipped.
            for _layer, sel in self.island_b.values():
                b_stmt = self.producer.get(sel.kwargs["b"])
                if b_stmt is None or b_stmt.op not in ("add", "mul"):
                    raise EncoderRunError(
                        f"layer {_layer}: producer of {sel.kwargs['b']} "
                        f"is {b_stmt.op if b_stmt else 'missing'}, not add/mul"
                    )
                if b_stmt.op == "add":
                    self.fused_ab_adds[b_stmt.index] = True
                else:
                    # One scalar-mul producer per layer; validate the chain
                    # once per layer here so apply-time cannot surprise.
                    add = self._fused_ab_consumer_add(sel)
                    if add is None:
                        raise EncoderRunError(
                            f"layer {_layer}: bdscale select output has no "
                            "unique add consumer"
                        )
                    self.fused_ab_adds[add.index] = True

    def _fused_ab_consumer_add(self, sel: Statement) -> Statement | None:
        """The unique add consuming the select's output (bdscale lineage:
        add_0 = matmul_N + attention_mask_9). None unless exactly one
        consumer exists and it is an add reading the select by name."""
        out_name = sel.names[0]
        users = []
        for stmt in self.statements:
            for token in stmt.kwargs.values():
                if out_name in self._operand_names(token.strip()):
                    users.append(stmt)
    ...
    def tensor(self, token: str) -> mx.array:
        token = token.strip()
        _t = time.monotonic_ns() if _stmt_trace else 0
        self.ensure(token)
        if _stmt_trace:
            _d = time.monotonic_ns() - _t
            if _d > 1_000_000:
                print(f"[ensure] {token} {_d/1e6:.2f} ms", flush=True)
        if token in self.values:
            value = self.values[token]
            if isinstance(value, _IslandFuture):
                value = value.wait()
    ...
    def scalar(self, token: str):
        ...
        """
        token = token.strip()
        # The protobuf re-export inlines attr literals as quoted strings;
        # the original adapter referenced const statements instead.
    ...
    def _eval_const(self, stmt: Statement) -> None:
        ...
        payload = self.blobs.payload_span(
            path, int(blob.group("offset")), count * itemsize
        )
        if payload % itemsize == 0:
            # Batched path: one device upload per (blob file, dtype); the
            # const is a slice view of it.
            dev, host = self.blobs.dtype_view(path, np_dtype)
            base = payload // itemsize
            view = dev[base : base + count]
            host_slice = host[base : base + count]
            self.values[name] = view.reshape(shape) if shape else view.reshape(())
            if not shape:
                self.meta[name] = (
                    float(host_slice[0]) if np_dtype != np.int32 else int(host_slice[0])
                )
            return
        raw = self.blobs.read_bytes(path, int(blob.group("offset")), count * itemsize)
    ...
    def execute(self, stmt: Statement) -> None:
        ...
        try:
            self._execute_inner(stmt)
        finally:
            _dt = time.monotonic_ns() - _t0
            self.seg_issue_ns += _dt
            entry = self.seg_ops.setdefault(stmt.op, [0, 0])
            entry[0] += 1
            entry[1] += _dt
            if _stmt_trace and _dt > 5_000_000 and stmt.op != "const":
                out = self.values.get(stmt.names[0])
                _sync = 0.0
                if out is not None and hasattr(out, "astype"):
                    _e = time.monotonic_ns()
                    mx.eval(out)
                    _sync = (time.monotonic_ns() - _e) / 1e6
                    self.seg_eval_ns += int(_sync * 1e6)
                print(
                    f"[stmt] {stmt.index:5d} {stmt.op:12s} {_dt/1e6:8.2f} ms"
                    f" eval-after={_sync:8.2f} ms",
                    flush=True,
                )

    def _execute_inner(self, stmt: Statement) -> None:
        if stmt.op == "const":
            self.eval_const(stmt)
            self.executed += 1
            return

        if self.fused_ab and stmt.index in self.fused_ab_adds:
            # The add is composed inside the fused A->B bundle.
            stmt.done = True
            self.executed += 1
            return
        if self.fused_ab and stmt.index in self.island_b:
            self._run_fused_ab(stmt)
            return
    ...
    def _feeder_checkpoint(self) -> None:
        """Snapshot the GPU-feeder segment since the last island submit
        (per-op CPU issue wall + counts) and hand it to the island for
        its log record."""
        if self.island is None:
            return
        self.island.feeder = {
            "issue_ns": self.seg_issue_ns,
            "ops": {op: [n, ns] for op, (n, ns) in self.seg_ops.items()},
        }
        self.seg_issue_ns = 0
        self.seg_ops = {}

    def _run_fused_ab(self, stmt: Statement) -> None:
        """Fused A->B: the add (scores + content) and the -inf mask
        select run as one out_ab ANE package (2 programs, composed
        intermediate), replacing the GPU add + GPU select pair.
        Contract (EHC out_ab): inputs s1, s2, fill, mask; output z."""
        layer, sel_stmt = self.island_b[stmt.index]
        b_stmt = self.producer[sel_stmt.kwargs["b"]]
        if b_stmt.op == "mul":
            # bdscale lineage (t8103-host): s1 = the scores matmul output via the
            # consumer add, s2 = the scaled relpos crop (GPU mul output).
            # z = select(fill, s1 + s2, mask) replaces the consumer add's
            # output; the select and the add never execute.
            add = self._fused_ab_consumer_add(sel_stmt)
            s1_name = next(
                n for token in add.kwargs.values()
                for n in self._operand_names(token)
                if n != sel_stmt.names[0]
            )
            s1 = self.tensor(s1_name)
            s2 = self.tensor(sel_stmt.kwargs["b"])
        else:
            add = b_stmt
            s1 = self.tensor(add.kwargs["x"])
            s2 = self.tensor(add.kwargs["y"])
        ...
        self._feeder_checkpoint()
        results = self.island.submit(
            "out_ab", f"L{layer:02d}-AB",
            {
                "s1": s1,
                "s2": s2,
                "fill": mx.contiguous(mx.broadcast_to(fill, ISLAND_B_SHAPE)),
                "mask": mx.contiguous(
                    mx.broadcast_to(cond, ISLAND_B_SHAPE).astype(mx.bool_)
                ),
            },
            {"z": (sel_stmt.shape, "fp16")},
        )
        if b_stmt.op == "mul":
            self.values[add.names[0]] = results["z"]
            add.done = True
        else:
            self.values[sel_stmt.names[0]] = results["z"]
        self.executed += 2
        self.ane_ops += 2
    ...
    def apply_conv(self, kwargs: dict) -> mx.array:
        ...
        x_raw = self.tensor(kwargs["x"])
        w_raw = self.tensor(kwargs["weight"])
        ...
        if (
            ...
            and w_raw.ndim == 3
            and w_raw.shape[2] == 1
            and x_raw.dtype == mx.float16
            and w_raw.dtype == mx.float16
            and "bias" not in kwargs
            and w_raw.shape[1] % 16 == 0
        ):
            # Pointwise 1x1 conv == linear over time. The f32 conv route
            # below upcasts x, weight AND bias and runs mx.conv2d in fp32
            # (24x2 dispatches, ~264 ms GPU/pass on T8103, plus ~300 MB of
            # f32 weight copies); the coopmat route keeps the same
            # fp16-in / one-final-fp16-rounding contract as the placed
            # linears. Battery-gated.
            n, c, t = x_raw.shape
            n_out = w_raw.shape[0]
            k = w_raw.shape[1]
            lhs = mx.reshape(mx.transpose(x_raw, [0, 2, 1]), (t, k))
            rhs = mx.reshape(w_raw, (n_out, k))
            blocks = k // 16
            partials = _linear_f16_coopmat_kernel()(
                inputs=[lhs, rhs],
                output_shapes=[(blocks, t, n_out)],
                output_dtypes=[mx.float16],
                grid=((n_out + 31) // 32 * 32, (t + 31) // 32, blocks),
                threadgroup=(32, 1, 1),
                stream=mx.gpu,
            )[0]
            out = _leftover_chain_kernel()(
            ...
            return mx.reshape(mx.transpose(out, [1, 0]), (n, n_out, t))
        x = x_raw.astype(mx.float32)
    ...
    def run(self, inputs: dict, wanted: set[str], stop_after: str) -> dict:
        ...
        keep = {
            name: (value.wait() if isinstance(value, _IslandFuture) else value)
