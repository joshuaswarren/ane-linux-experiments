#!/bin/bash
# ab-inproc-j1.sh — interleaved A/B on j1: resident-batch (worker
# fallback) vs inprocess (ane_inproc shim). One runner file, mode via env.
# Lock protocol identical to ab-battery-j1.sh: flock /tmp/m1-gpu.lock.
set -uo pipefail
...
BASE=/var/tmp/encwall-decomp/inproc-$TS
mkdir -p "$BASE"
export MLX_OMARCHY_PLACED=AC
unset PYTHONPATH LD_LIBRARY_PATH HK_PERF HK_PERFTEST MLX_OMARCHY_GATED_BARRIERS MLX_OMARCHY_GPU_PROFILE ANE_OP_WALL
{
  date -Ins; hostname
  sha256sum "$WORKER" "$LIBANE" "$SHIM" "$RUNNER" "$RUN/fused_e2e.py" "$PY"
} > "$BASE/identity.txt" 2>&1

run_one () { # arm mode name
  local arm=$1 mode=$2 name=$3
  local out=$BASE/out-$arm-$name scratch=$BASE/scratch-$arm-$name
  rm -rf "$out" "$scratch"; mkdir -p "$out" "$scratch"
  ANE_ISLAND_MODE=$mode ANE_INPROC_SHIM=$SHIM \
  "$PY" "$RUN/fused_e2e.py" \
...
run_one B inprocess warm-smoke || { echo "SMOKE-FAILED"; exit 3; }

run_one A resident-batch warm || exit 3
run_one B inprocess warm || exit 3
for i in 1 2 3 4 5 6; do
  run_one A resident-batch "meas-$i" || exit 3
  run_one B inprocess "meas-$i" || exit 3
done
...
for d in sorted(glob.glob(base+"/out-*")):
    ...
    agg = {}
    for e in log:
        b = e["bundle"]; a = agg.setdefault(b, dict(n=0, marshal=0.0, el=0.0))
        a["n"] += 1
...
summary = {"schema":"j1-inproc-ab/1","base":base,"arms":{}}
