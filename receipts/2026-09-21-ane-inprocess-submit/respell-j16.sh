#!/bin/bash
# respell-j16.sh — interleaved A/B: resident-batch (worker fallback) vs
...
WORKER=/var/tmp/j16-oproj-place/mlx-omarchy-ane-worker
LIBANE=/var/tmp/j16-oproj-place/libane-strict-fill.so
SHIM=/var/tmp/encwall-decomp/libane_inproc.so
RUNNER=/var/tmp/encwall-decomp/inproc-tmp/vulkan_encoder_inproc.py
TS=$(date +%Y%m%dT%H%M%S)
BASE=/var/tmp/encwall-decomp/respell-$TS
mkdir -p "$BASE"
...
run_one () { # arm mode name fused source
  local arm=$1 mode=$2 name=$3 fused=$4 source=$5
  ...
  "$PY" "$RUN/fused_e2e.py" \
    ...
    --bundles /var/tmp/j16-encoder-islands/bundles \
    --worker "$WORKER" \
    --libane "$LIBANE" \
...
run_one B inprocess 1 warm-smoke /var/tmp/EncoderParityAne/encoder-source-respell || { echo "SMOKE-FAILED"; exit 3; }

run_one A inprocess 1 warm /var/tmp/EncoderParityAne/encoder-source || exit 3
run_one B inprocess 1 warm /var/tmp/EncoderParityAne/encoder-source-respell || exit 3
for i in 1 2 3 4 5 6; do
  run_one A inprocess 1 "meas-$i" /var/tmp/EncoderParityAne/encoder-source || exit 3
  run_one B inprocess 1 "meas-$i" /var/tmp/EncoderParityAne/encoder-source-respell || exit 3
done
