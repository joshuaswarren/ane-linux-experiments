# Parakeet encoder wall decomposition + instrumentation, jwm1 and jw16 (2026-09-21)
...
Remote artifacts: `/var/tmp/encwall-decomp/` on jwm1 and jw16.

## Decomposition (per encoder pass, medians over measured runs)

### jwm1 (T8103) — perf-battery-20260921T062634, 10 meas runs
...
Copy microbench (jwm1 venv python): 3.8 MB tobytes 0.087 ms; bytearray
append 0.296 ms — host copies negligible (~20-30 ms/pass). Marshal is
GPU-feeder eval wait, not copying.

### jw16 (T6001) — parakeet-parity-battery-20260921T091631, 10 meas runs
...
- jwm1 5243.3 / 259.9 (macOS 27 same-encoder, CoreML context) = 20.2x;
  vs 113.0-115.8 ms native figure = 45-46x.
- jw16 3329.5 / 292.2 (cross-chip M1 Ultra/T6000; T6001 native divisor
  pending) = 11.4x.
...
3. **Client IPC tuning: measured, REJECTED.** Interleaved 1 warm + 5 meas
   A/B on jw16 (/var/tmp/encwall-decomp/ab-20260921T165936): base 3486.7
...
patched workers: jwm1 /var/tmp/encwall-decomp/worker-cli-v2 (4d52ae9c),
jw16 /var/tmp/encwall-decomp/worker-cli-v2-16 (65aab8c1).

### Raw write(2) emit (patch in main.cpp.orig [jwm1] / main-jw16.cpp [jw16])
- jwm1 ANE-only bench, 24 rounds x 4 interleaved pairs: old median 1156.0
  vs patched 1116.7 ms (~ -3%), patched faster 3/4 pairs, tighter variance.
- jw16 same protocol: old median 667.1 vs patched 647.8 ms (~ -3%).
- Direction consistent both hosts; ~3% of ANE path, within run-to-run
  ...
  fused_e2e battery with the patched worker (recipe: ab-battery-jw16.sh
  with WORKER=/var/tmp/encwall-decomp/worker-cli-v2-16), blocked only on
  the jw16 GPU lock window (GPUHardwareContinuation queue).
...
- /tmp/mesa-sin-ftz-jwm1 (libvulkan_asahi-e167) wiped with /tmp; stock
  mesa 26.2.3 segfaults the pipeline (rc=139, ~8 GPU submits). jwm1
  fused_e2e blocked until GPUHardwareContinuation's e167 rebuild lands.
- jwm1 worker rebuild via plain ninja hits STAGING_BYTES (see fix above).
- jw16 llama-server (qwen3.8-27b:8002) stopped for measured windows and
...
jw16 full fused_e2e battery with worker-cli-v2-16 (raw write(2) emit,
...
main.cpp (patch: main.cpp.orig [jwm1 lineage] / main-jw16.cpp [jw16]),
