...
NDJSON="$GDB/curve-$STAMP.ndjson"
IDENT="$GDB/curve-$STAMP.identity.json"
REPORT="$GDB/curve-$STAMP.report.json"
SUMS="$GDB/curve-$STAMP.sha256"
...
"$V" "$GDB/qmm_weight_curve_micro.py" --pass curve-report \
  --profile "$NDJSON" \
  --plan-meta "$IDENT" \
  --out "$REPORT"
