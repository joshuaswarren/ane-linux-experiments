...
| cadence prefill-512 (10×3, greedy, pinned digest) | ttft 62.25, pure prefill 267.7 (+12% from GDN prefill rework vs 238.3), decode 56.8, digest `ac1b269553a220ee…` | fork driver: ttft 46.68, pure prefill 128.66, decode 34.3, digest `ac1b2695…` = m1max-host, byte-identical; second leg 46.7/128.3/34.29, same digest — **not a stock-Mesa leg**: it ran with `VK_DRIVER_FILES` unset, which loads the system-default ICD, and the fork IS the system default (`asahi_icd.json` → `libvulkan_asahi.so.7faf04c`; stock Mesa 26.2.3 parked as `asahi_icd.json.stock-mesa-2623.bak`). Both legs ran the fork. The 32.9 tok/s stock-Mesa figure is the earlier PrefillProfileAttack measurement, not from this battery. Stock Mesa reaches 128 only in the sense that it was never exercised here — no claim is made. |
