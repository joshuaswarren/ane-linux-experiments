...
def encode(prompt):
    ...
    # brew llama.cpp prints a python-list ("[1, 2, ...]"), some builds print
    # bare ints; take every integer either way.
...
def build_result(records, pure_prefill, peak_rss):
    result = {
        "meta": {
...
def checkpoint(records):
    if a.out:
        ...
        with open(a.out + ".partial", "w") as f:
            json.dump(build_result(records, None, rss), f, indent=1)
...
result = build_result(records, pure_prefill, peak_rss)
