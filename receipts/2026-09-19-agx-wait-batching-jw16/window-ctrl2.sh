#!/usr/bin/env bash
# Control 2: installed-driver arm (hk5deac1c-2 package) — same 3 short runs.
exec > /var/tmp/cb/window-ctrl2.log 2>&1
set -uo pipefail
L=/var/tmp/cb
cleanup() {
...
export VK_DRIVER_FILES=/var/tmp/MesaL2/pkg-installed/icd.json
for i in 1 2 3; do
