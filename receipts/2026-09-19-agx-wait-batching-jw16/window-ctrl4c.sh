#!/usr/bin/env bash
# Control 4c: warm-cache confirmation of AGX_SIMDMAT=1 on the trunk build.
# Disk shader cache now warm from ctrl4b; 4 runs, expect a settled ~18x.
exec > /var/tmp/cb/window-ctrl4c.log 2>&1
set -uo pipefail
L=/var/tmp/cb
