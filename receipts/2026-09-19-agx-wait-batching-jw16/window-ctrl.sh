#!/usr/bin/env bash
# 90-second control: is the 141 tok/s short level thermal contamination from
# the -j10 ninja build, or a real box-state change? 3 rounds, base arm, short leg.
exec > /var/tmp/cb/window-ctrl.log 2>&1
set -uo pipefail
L=/var/tmp/cb
cleanup() {
...
date -u +%FT%TZ
sensors 2>/dev/null | grep -iE "package|edge|soc|fan" | head -8
...
for i in 1 2 3; do
  $PY $B --model "$MODEL" --prompt "Hi" --tokens 32 \
    --temp 0.0 --seed 0 --warmup-tokens 4 2>&1 | grep -E "^decode|^prefill|generated_ids"
done
