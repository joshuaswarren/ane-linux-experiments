...
`dc7ca4a0`. Local branch `agent/decode-m1max-occupancy`: jw16 worktree
`/var/tmp/occ-wt` commit `92d71186`, jwm1 worktree `/var/tmp/occ-wt` commit
...
- **jwm1 saturates at ~2048–4096 subgroups** (floor ≈ 10 ns/subgroup,
...
- **jw16 is still climbing at 131072 subgroups** (310 GB/s pattern roof);
...
- Per-dispatch reality check: the honest in-model instrument (gap,
  widedep 24-set chain) gives production **160.0 GB/s jw16 /
  48.5 GB/s jwm1** — i.e. the shipped kernel already runs AT the pattern
...
| candidate | bit-identical | jw16 widedep GB/s | jwm1 widedep GB/s | verdict |
| --- | --- | ---: | ---: | --- |
| production (xpack) | — | **160.0** | **48.5** | baseline (xpack arm = base to 0.03%, instrument honest) |
| w2 — uvec2 128-bit weight loads | yes (0 mismatches, 28 eq rows ×2 hosts) | n/a (isolated flat) | flat (−1%) | REJECT — pattern probe: wide loads are *slower* (v2 148.6 / v4 139.8 vs w2 161.8 at 8192 subgroups, jw16) |
...
| --- | --- | ---: | ---: | --- |
| jw16 (M1 Max) | `/var/tmp/occ-venv` = `0.32.3.dev202609221028+dc7ca4a0` | **57.67** | 57.59 (integration receipt) | `cadence-jw16-integ.json` |
| jwm1 (M1) | `/var/tmp/integ-venv` = same dc7ca4a0 wheel | **34.14** | 34.30 | `cadence-jwm1-integ.json` |
...
- jw16 llama-server stopped for the bench window (pid 217612) and
  restored with the recorded cmdline (new pid 248451; needed
  `LD_LIBRARY_PATH=/home/joshuawarren/opt/llama-vulkan`, which the
  recorded cmdline alone lacked — worth noting in the restore file);
  `GET /health` = 200 after. jwm1: no llama running (as before); all
  jwm1 runs under `flock /tmp/m1-gpu.lock`. jw14m2 untouched. No pushes.
- Instrument limitation recorded for the next lane: jw16 wall-anchored
  ...
  Isolated single-dispatch kernel timings on jw16 are also floor- and
