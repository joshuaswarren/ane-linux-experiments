...
def describe_layout(buf: bytes) -> str:
    """Human-readable layout of one e5 program (studio-host oproj is canonical)."""
...
def main() -> None:
    ...
    for g in GEOMS:
        a, b = e5[("studio-host-t6000-host", g, "h14g")], e5[("t6021-test-host-t6021-host", g, "h14g")]
        ...
        report["cross_host"][g] = {"aligned_shift": best[0], "differing": len(diff_idx),
                                   ...
                                   "chip_studio-host": f"0x{chip_a:04x}",
                                   "chip_t6021-test-host": f"0x{chip_b:04x}"}
    ...
    a = e5[("studio-host-t6000-host", "mm1", "h14g")]
    for g in ("oproj", "mm2"):
        b = e5[("studio-host-t6000-host", g, "h14g")]
        rs = diff_ranges(a, b)
    ...
    for g in GEOMS:
        payload = load_weights(mints, "t6021-test-host-t6021-host", g)[0x80:]
    ...
    o, m1 = e5[("studio-host-t6000-host", "oproj", "h14g")], e5[("studio-host-t6000-host", "mm1", "h14g")]
    ...
    print(f"\nverdict JSON -> {out}")
    print(describe_layout(e5[("studio-host-t6000-host", "oproj", "h14g")]))
