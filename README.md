...
T8103 on m1-test-host has a bound ANE. Persistence is `m1-test-host-ane.service`. Schema-4 add-mul, tiny select, 1x1 conv, and `(1,256,128)` linear are exact fp16. 1x896 times out with -110 and is forbidden.

T6001 on t6001-test-host has `/dev/accel/accel0` live. SET0 gates on runtime suspend and returns `0xf` on `open(accel0)` via genpd. Do not write SET `0xf`. The same add-mul, tiny select, 1x1 conv, and `(1,256,128)` linear are exact and match T8103. 1x896 remains forbidden. A live overlay is not the packaged DTB.
