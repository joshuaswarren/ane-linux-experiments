# jwm1 (T8103 MacBook Pro) — Asahi 1TR boot-object repair runbook
...
`receipts/2026-09-18-jwm1-macos-root-deadlock.md` in this repo.

## Situation (why this is needed)

After the jwm1 macOS 26.6.2 → 27.0 OTA, the Asahi stub chain
...
2. From any fleet Linux box, run:
   ...
   ```bash
   bash scripts/recover-jwm1.sh --wait 1800
   ```
...
## Phase 3 — when jwm1-linux returns

From any fleet Linux box:

```bash
bash scripts/v070-jwm1-arm.sh --wait 86400
```

One command: stages the jw16-proven gate kit (wheel sha `2def345c…`,
oproj islands, probes, E2E harness) onto jwm1-linux and runs the both-host
...
- Fleet receipts: `receipts/2026-09-18-t8103-divisor-macos27.md` ADDENDUM 1+2
  ...
  `receipts/2026-09-18-jwm1-macos-root-deadlock.md` (root/deadlock analysis),
