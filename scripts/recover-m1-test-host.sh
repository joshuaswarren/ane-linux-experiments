#!/bin/bash
# m1-test-host one-command recovery — run from any fleet Linux box the moment the
# macOS side of m1-test-host answers (tailnet [redacted-ip] / alias jw-m1-macos, or
# LAN [redacted-ip]). Does everything remotely; the owner's only local work
# afterwards is the Asahi 1TR repair at the machine (runbook below).
#
#   scripts/recover-m1-test-host.sh              # do it now
#   scripts/recover-m1-test-host.sh --wait 1800  # poll for the host up to 30 min
#
# Steps:
#  1. reach m1-test-host macOS (tailscale first, then LAN)
...
set -u
TS_HOST=[redacted-ip]       # tailnet (jw-m1-macos alias)
LAN_HOST=[redacted-ip]      # LAN fallback
USER=joshuawarren
RUNBOOK_SRC="$(dirname "$0")/m1-test-host-asahi-1tr-runbook.md"
ARM_RUNNER="$(dirname "$0")/v070-m1-test-host-arm.sh"
...
until [ -n "$HOST" ]; do
  ...
  [ -n "$HOST" ] && break
  [ "$WAIT" -gt 0 ] || die "m1-test-host macOS not reachable on $TS_HOST or $LAN_HOST. Boot it (iBoot picker -> Macintosh HD) or pass --wait N."
...
scp -q -o ConnectTimeout=6 "$RUNBOOK_SRC" "$USER@$HOST:~/Desktop/m1-test-host-asahi-1tr-runbook.md" \
  && note "runbook staged: ~/Desktop/m1-test-host-asahi-1tr-runbook.md on m1-test-host (also in this repo: $RUNBOOK_SRC)" \
...
NEXT STEPS
  1. At the machine (or via the staged copy): follow the Asahi 1TR repair
     runbook to rebuild the boot-object chain and bring m1-test-host-linux back:
       $RUNBOOK_SRC
  2. When m1-test-host-linux ([redacted-ip]) is back, run the both-host v0.7.0 arm:
       $ARM_RUNNER
