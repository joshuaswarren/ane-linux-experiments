#!/bin/bash
# v0.7.0 BOTH-HOST gate — jwm1-linux (T8103, M1) arm. Mirror of the jw16
# V070Recert gate (/tmp/v070-jw16-gate.sh, receipt
# mlx-omarchy/receipts/2026-09-18-v070-pretag-recert-jw16.md) with
# jwm1-linux paths and the C++ suites added. Run by scripts/v070-jwm1-arm.sh,
# which stages $ROOT/{wheel,kit,oproj} and resolves the E2E inputs first.
# Runs ON jwm1-linux.
#
# Gates (identical pins to jw16):
...
#  (1) clean-HOME packaging 3x (gate-jwm1.sh): transcript db501a8c... each run.
...
ROOT=/var/tmp/v070-jwm1
KIT="$ROOT/kit"
GATE="$KIT/gate-jwm1.sh"
...
# E2E inputs (runner resolves and exports these; jw16-canonical defaults)
...
pgrep -f "mlx-omarchy-parakeet|gate-jwm1|bench_matrix|fused_e2e" >/dev/null \
...
# ACO provenance (same finding as jw16, receipt above): the wheel ships no
...
echo "$LIBMLX16 v0.7.0-candidate-b283a16 (LOCAL candidate line, pre-tag, jwm1 arm)" \
