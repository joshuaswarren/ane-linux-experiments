...
#     against the 2026-09-17 studio-host/t6021-test-host tool (52336 B,
...
ARCHIVED_OPROMJ_SHA16="9351e29807fcaf34"   # t6021-test-host oproj h14g, 2026-09-17
...
if [ -z "$AF_DIR" ] && ssh -o BatchMode=yes -o ConnectTimeout=6 \
    studio-host true >/dev/null 2>&1; then
  say "fetching aneforge scratch from studio-host (BatchMode)"
  if rsync -a studio-host:src/ANEForge/ /tmp/ANEForge-scratch/ \
...
if [ -n "$AF_DIR" ] && [ -z "$PY" ]; then
  ...
  # 2026-09-17 t6021-test-host mint run.
