---
title: MLX on Omarchy Vulkan and ANE - Plan
type: feat
date: 2026-08-29
deepened: 2026-08-29
...
# MLX on Omarchy Vulkan and ANE - Plan
...
- **Means:** Maintain an upstream-following MLX fork with a Vulkan GPU backend and an internal ANE graph accelerator. (KTD1, KTD2, KTD3)
- **Authority:** The Product Contract governs behavior. The Key Technical Decisions govern implementation within that contract.
- **Execution profile:** Prove GPU compatibility first. Add ANE only after the current full-graph parity gate passes.
...
The Apple GPU supplies the complete tensor baseline through Vulkan.
...
- **Ship one new repository.** Create `mlx-omarchy` for the maintained MLX fork, integration code, packages, and user documentation. Keep ANE experiments here and send driver changes to `eiln/ane`. Governs R12, R15, R19. (session-settled: user-approved — chosen over several empty product repositories: one working codebase is easier to build and maintain)
...
- A Vulkan-only compatibility release may ship after U5, but it does not complete this hybrid plan.

### Scope Boundaries

**Included**

- M1 Apple Silicon on Omarchy and the Omarchy Linux graphics stack.
...
- KTD1. **Maintain an MLX fork against a pinned upstream baseline.** MLX selects complete GPU backends at compile time and has no external GPU plug-in boundary. `mlx-omarchy` will record an upstream release tag and commit, track upstream, and add `MLX_BUILD_OMARCHY` beside Metal and CUDA. Every release syncs to a named baseline and reruns U2 through U4. This implements R1 through R5.
...
- KTD8. **Keep the repository set small.** `mlx-omarchy` owns the MLX fork, integration, packaging, and release docs. `ane-linux-experiments` remains the hardware evidence lab. Driver ABI changes belong in `eiln/ane`. This implements R12 and R15.
...
- **Goal:** Make MLX allocate, schedule, synchronize, and copy arrays through Honeykrisp Vulkan, then prove the core kernel performance bet.
- **Requirements:** R3, R4, R5, R7, R8, R16
- **Repository:** `mlx-omarchy`
- **Files:** `CMakeLists.txt`, `mlx/backend/omarchy/allocator.cpp`, `mlx/backend/omarchy/device.cpp`, `mlx/backend/omarchy/eval.cpp`, `mlx/backend/omarchy/event.cpp`, `mlx/backend/omarchy/stream.cpp`, `mlx/backend/omarchy/vulkan.h`, `tools/mlx-omarchy-info/`, `benchmarks/omarchy/matmul_attention.cpp`, `tests/omarchy/test_runtime.cpp`
- **Patterns:** Follow the full backend boundary in `mlx/backend/cuda/` and the no-backend failure contracts in `mlx/backend/no_gpu/`.
- **Approach:** Add `MLX_BUILD_OMARCHY`. Implement device selection, host-visible unified-memory buffers, command pools, queues, events, fences, copies, backend trace records, and the capability report. Before broad U3 work, implement the minimum matmul and attention kernels needed for matched prefill and decode spikes. Stop and revise the backend design unless both reach at least 80 percent of the pinned same-machine `llama.cpp` Vulkan reference.
- **Test scenarios:**
  ...
  - The matched matmul and attention spikes record the pinned comparator, prompts, shapes, quantization, thermal procedure, and 80 percent go/no-go result.
- **Verification:** `tools/ci/run-omarchy-runtime.sh`, `tools/ci/run-omarchy-spike.sh`, and `mlx-omarchy-info --trace-smoke` on the target machine with zero CPU primitive dispatches.
- **Dependencies:** None.
...
### U8. Qualify real MLX programs and publish releases

- **Goal:** Prove the port through useful command-line programs and servers, then publish reproducible artifacts.
...
- **Dependencies:** U5 for the Vulkan release. U7 for ANE-enabled release artifacts.
