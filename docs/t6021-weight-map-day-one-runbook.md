# Day-one runbook: t6021 weight-plane map device verification (jw14m2 dual-boot)
...
maps), so it must be measured on the device. Everything below runs on jw14m2 booted
...
| item | path | note |
|---|---|---|
| oproj watermark weights | `receipts/2026-09-17-t6021-h14g-oracle-mints/jw14m2-t6021-host/oproj/weights.bin` | identity-ordered pair28 payload at 0x80; sha256 8b4a1451… |
...
| fixed-path pair | `…/insurance/fixed-path-experiment/fixed-{jw14m2-H14C,macstudio-H13D}.e5` | clean 1-byte family delta |
...
1. Build programs with the mlx-omarchy exporter exactly as for jwm1 (its
...
2. Load with the repo's libane stack (`patches/libane-ane.c`:
   ...
   `__ane_read`), following the established jwm1 gate procedure.
...
4. Interpret:
   - All groups rel_l2 ≈ 0.01-level (the jwm1 post-fix level) → **H13 maps hold on
