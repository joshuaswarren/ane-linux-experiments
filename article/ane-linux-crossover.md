...
For years this comparison ran the other way. My last receipt for this exact head said 0.33x. Running the ANE from Linux has a graveyard of abandoned forks behind it: eiln/ane, apple-ane, each one getting partway in and stopping. This post is the story of the last two days on m1-test-host-linux, a 16 GB M1 MacBook Pro running Asahi Linux under Omarchy. The repo is public at github.com/joshuaswarren/ane-linux-experiments, and every number here has a line in receipts/ane-static-graph-loop.log.
...
```
sudo ANE_KO=$HOME/ane-boot/ane-qid.ko bash ~/ane-boot/m1-test-host-ane-bringup.sh
```
