...
# /var/tmp/ane-runtime/fused-e2e/baseline-reconfirm-TS/.
#
...
BASE=/var/tmp/ane-runtime/fused-e2e/baseline-reconfirm-$TS
LOG=$BASE/run.log
mkdir -p "$BASE"
chmod 755 "$BASE"
...
# Identity pin before any work — verifies the binary state at run time.
{
  ...
  sha256sum \
    ...
    /var/tmp/encoder-overlay/base/vulkan_encoder.py \
    /var/tmp/ane-runtime/fused-e2e/fused_e2e.py \
