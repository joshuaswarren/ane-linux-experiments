#!/bin/bash
# lease_run.sh — Bounded jwm1 /dev/accel/accel0 lease for ParakeetPerformance.
# Protocol: acquire /tmp/m1-gpu.lock inode 27 (flock, 1200s ceiling),
# run perf-battery.sh (warm + 10 measured), release lock. Output to
# /var/tmp/jwm1-ane-step2/fused-e2e/baseline-reconfirm-TS/.
#
...
BASE=/var/tmp/jwm1-ane-step2/fused-e2e/baseline-reconfirm-$TS
LOG=$BASE/run.log
mkdir -p "$BASE"
chmod 755 "$BASE"
...
# Identity pin before any work — verifies the binary state at run time.
{
  ...
  sha256sum \
    ...
    /var/tmp/jwm1-ane-step2/fused-e2e/fused_e2e.py \
