...
# Protocol per Main + t6001-test-host window script: 1 warm + 10 measured runs.
...
RUN=/var/tmp/m1-test-host-ane-step2/fused-e2e
...
DRIVER=/tmp/mesa-sin-ftz-m1-test-host/drivers/libvulkan_asahi-e167.so
ICD=/tmp/mesa-sin-ftz-m1-test-host/m1-test-host-e167-icd.json
TS=$(date +%Y%m%dT%H%M%S)
BASE=/var/tmp/m1-test-host-ane-step2/fused-e2e/perf-battery-$TS
mkdir -p "$BASE"
