#!/bin/bash
# Mode-default re-baseline on the flipped certified copy.
# Arms: AC/ACO x {serve-default (no env), launch (env)} x3. One window.
set -uo pipefail
...
run_arm () { # name placed mode_env rep  # mode_env: "" (default) or "launch"
  local name=$1 placed=$2 mode_env=$3 rep=$4
  local outdir=$W/out-md-$name scratch=$W/scratch-md-$name
  echo "--- arm $name PLACED=$placed MODE_ENV='$mode_env' rep=$rep $(date -Iseconds) ---"
  ...
  if [ -n "$mode_env" ]; then
    export ANE_ISLAND_MODE=$mode_env
  else
    unset ANE_ISLAND_MODE || true
  fi
  MLX_OMARCHY_PLACED="$placed" \
  ...
  $PY $RUN/fused_e2e.py \
    ...
    --scratch $scratch \
    --out $outdir \
    --deadline-ms 20000 > $W/md-$name.log 2>&1 || { echo "ARM $name FAILED"; tail -5 $W/md-$name.log; return 1; }
  unset ANE_ISLAND_MODE || true
  python3 - "$outdir" "$name" "$placed" "$mode_env" "$rep" <<'PY'
import hashlib, json, sys
out, name, placed, mode_env, rep = sys.argv[1:6]
...
row = {
    "arm": name, "placed": placed, "mode_env": mode_env, "rep": rep,
    "mode": mode_env or "serve-default",
...
with open("/var/tmp/jw16-spawn-attrib/modedefault.jsonl", "a") as f:
    f.write(json.dumps(row) + "\n")
PY
}
for rep in 1 2 3; do
  run_arm "ac-serve$rep" AC "" $rep || true
  run_arm "ac-launch$rep" AC launch $rep || true
  run_arm "aco-serve$rep" ACO "" $rep || true
  run_arm "aco-launch$rep" ACO launch $rep || true
done
echo "MODEDEFAULT DONE"
