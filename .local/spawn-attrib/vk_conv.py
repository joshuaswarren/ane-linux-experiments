...
class AneIsland:
    ...
    def __init__(self, worker: Path, libane: Path, bundles: Path, scratch: Path,
        ...
        self.log: list[dict] = []
        # MLX_OMARCHY_LAUNCH_PROFILE=1: env-gated per-phase timing, sums only.
        ...
        if self.profile and self._profile_out:
            ...
            def _dump(island=self):
                import json as _json

                with open(self._profile_out, "w") as f:
                    _json.dump(
                        {
                            "mode": island._mode,
    ...
    def _submit_resident(self, bundle: str, tag: str, inputs: dict, outputs: dict) -> dict:
        ...
        for name, (shape, dtype_name) in outputs.items():
            ...
            if self.profile:
                self.phase_ns["convert"] += time.monotonic_ns() - t0
    ...
    def _submit_launch(self, bundle: str, tag: str, inputs: dict, outputs: dict) -> dict:
        ...
        for name, value in inputs.items():
            ...
            t_write += time.monotonic_ns() - t0
            in_bytes += raw.nbytes
            argv += ["--input", f"{name}={path}"]
        if self.profile:
            self.phase_ns["eval"] += t_eval
            self.phase_ns["write"] += t_write
        saved = {}
        for name in outputs:
        ...
        elapsed = time.monotonic_ns() - started
        if self.profile:
            self.phase_ns["spawn"] += elapsed
            self.per_bundle_ns.setdefault(bundle, []).append(elapsed)
        self.submissions += 1
        self.exec_ns += elapsed
        self.input_bytes += in_bytes
        ...
        for name, (shape, dtype_name) in outputs.items():
            ...
            t_conv += time.monotonic_ns() - t0
            saved[name].unlink()
        if self.profile:
            self.phase_ns["read"] += t_read
            self.phase_ns["convert"] += t_conv
        self.output_bytes += out_bytes
        record["input_bytes"] = in_bytes
        record["output_bytes"] = out_bytes
...
def main() -> int:
    ...
    if island is not None:
        report["ane"] = {
            ...
            "exec_ns": island.exec_ns,
            "profile": (
                {"phase_ns": island.phase_ns, "per_bundle_ns": island.per_bundle_ns}
                if island.profile
                else None
            ),
            "log": island.log,
