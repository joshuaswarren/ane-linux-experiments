#!/bin/bash
# Batch-eval measurement: MLX_OMARCHY_PIPE=1 MLX_OMARCHY_PIPE_OPS=conv, AC/ACO x launch/resident x3.
...
run_arm () { # name placed mode rep
  ...
  MLX_OMARCHY_PLACED="$placed" ANE_ISLAND_MODE=$mode MLX_OMARCHY_PIPE=1 MLX_OMARCHY_PIPE_OPS=conv \
...
with open("/var/tmp/jw16-spawn-attrib/pipeconv.jsonl", "a") as f:
