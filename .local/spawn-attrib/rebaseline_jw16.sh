#!/bin/bash
# Re-baseline on the flipped certified copy /tmp/conv-lane/vk_conv.py.
# Arms: AC and ACO, launch + resident, x3, interleaved. One window.
set -uo pipefail
...
run_arm () { # name placed mode rep
  ...
  local outdir=$W/out-rb-$name scratch=$W/scratch-rb-$name
  ...
  $PY $RUN/fused_e2e.py \
    ...
    --scratch $scratch \
    --out $outdir \
    --deadline-ms 20000 > $W/rb-$name.log 2>&1 || { echo "ARM $name FAILED"; tail -5 $W/rb-$name.log; return 1; }
...
with open("/var/tmp/jw16-spawn-attrib/rebaseline.jsonl", "a") as f:
