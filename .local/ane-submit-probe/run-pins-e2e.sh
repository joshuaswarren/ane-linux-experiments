#!/bin/bash
# run-pins-e2e.sh — one pin-gated e2e pass on the certified green config
# (WheelR4V062's R4 invocation, PLACED=ABC, resident-batch). Caller holds
# /tmp/m1-gpu.lock. Gates checked by /var/tmp/jw16-ep-bisect/gate.py.
set -uo pipefail
TAG=${1:?tag required}
...

{
  ...
  echo "runner: $(sha256sum /var/tmp/jw16-r4/vulkan_encoder_r4.py)"
  echo "worker: $(sha256sum /var/tmp/r4-wheelx/mlx/bin/mlx-omarchy-ane-worker)"
  echo "libane: $(sha256sum /var/tmp/jw16-oproj-place/libane-strict-fill.so)"
} > "$OUTDIR/identity.txt"
...
$PY $RUN/fused_e2e.py \
  ...
  --encoder-runner /var/tmp/jw16-r4/vulkan_encoder_r4.py \
...
python3 /var/tmp/jw16-ep-bisect/gate.py "$OUTDIR" 38c73261 || echo "GATE FAILED for $TAG"
