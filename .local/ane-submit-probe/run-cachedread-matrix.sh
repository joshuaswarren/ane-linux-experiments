#!/bin/bash
# run-cachedread-matrix.sh — A0 uncached-read-back A/B on jw16.
# MUST run inside the device window: /tmp/m1-gpu.lock held (caller wraps),
# llm-inference stopped, ane refcnt 0. One module swap (original -> map_mode
# variant); every mode switch after that is a runtime param write.
# Every verify2 must produce hashes and be STABLE within-process; mode-3
# hashes must equal the original-module baseline byte-for-byte.
set -uo pipefail
P=/var/tmp/ane-submit-probe
PROBE=$P/ane-probe2
OUT=$P/cachedread-out
ORIG_KO=/usr/local/lib/omarchy-ane/ane.ko
VARIANT_KO=/home/joshuawarren/src/ane-cachedread-wt/ane/ane.ko
CTRL_BUNDLE=/var/tmp/jw16-ane-first-exec/bundle
RX=$P/bundles-rx
PROGS="ctrl0=$CTRL_BUNDLE/program-0.anec ctrl1=$CTRL_BUNDLE/program-1.anec a0=$RX/island-attn-a-kt/program-0.anec a1=$RX/island-attn-a-kt/program-1.anec sel=$RX/island-select-8head-scratch417/program-0.anec pv=$RX/island-pv/program-0.anec"
DECOMPS="a0=$RX/island-attn-a-kt/program-0.anec a1=$RX/island-attn-a-kt/program-1.anec sel=$RX/island-select-8head-scratch417/program-0.anec pv=$RX/island-pv/program-0.anec"
mkdir -p "$OUT"
...
# Fails hard on: no hashes, within-process divergence.
run_v2() {
