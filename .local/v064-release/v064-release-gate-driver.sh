#!/bin/bash
# v0.6.4 release gate on jwm1 — corrected driver.
...
#   - decode leg uses dedicated V064REL venv + HF_HUB_OFFLINE=1
set -u
URL="https://github.com/joshuaswarren/mlx-omarchy/releases/download/v0.6.4/mlx_omarchy-0.32.2.dev202609171442+112c32c-cp314-cp314-linux_aarch64.whl"
EXPECT_SHA="a2ddd18824610be0bd5b927822efc5bcb3eb463c41502fcb92016fe26bbd8a46"
...
OUT=/var/tmp/v064-release-gate
STATUS=/var/tmp/v064-release.status
...
curl -fsSL --retry 3 --max-time 600 -o "$OUT/mlx_omarchy-0.32.2.dev202609171442+112c32c-cp314-cp314-linux_aarch64.whl" "$URL" || fatal "release download failed"
sha=$(sha256sum "$OUT/mlx_omarchy-0.32.2.dev202609171442+112c32c-cp314-cp314-linux_aarch64.whl" | cut -d' ' -f1)
...
bash "$GATE" "$OUT/mlx_omarchy-0.32.2.dev202609171442+112c32c-cp314-cp314-linux_aarch64.whl" "$OUT/packaging" > "$OUT/packaging.log" 2>&1
gate_rc=$?
...
"$VENV/bin/pip" install --quiet --no-deps --force-reinstall "$OUT/mlx_omarchy-0.32.2.dev202609171442+112c32c-cp314-cp314-linux_aarch64.whl" || fatal "wheel install failed"
